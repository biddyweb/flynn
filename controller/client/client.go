...
func (c *Client) StreamDeployment(deploymentID string, output chan *ct.DeploymentEvent) stream.Stream {
...
func (c *Client) DeployAppRelease(appID, releaseID string) error {
	...
	stream := c.StreamDeployment(d.ID, events)
	defer stream.Close()
outer:
	for {
		select {
...
func (c *Client) StreamJobEvents(appID string, output chan *ct.JobEvent) stream.Stream {
